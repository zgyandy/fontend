...
export default {
  ...
  mounted () {
    if (location.host.indexOf('test') != -1) {
      ...
      this.$store.commit('vipUrl', 'http://portal.anaiotek.com/payment/checkVip')
      this.$store.commit('forumCode','http://192.168.8.236:8002')
      this.$store.commit('domain', 'http://localhost:8082')
    }
  },
  watch: {
    $route: function (to) {
